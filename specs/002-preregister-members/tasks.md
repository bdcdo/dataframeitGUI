...
- [ ] T008 [US1] Nova action `updatePendingMemberEmail(projectId, memberUserId, newEmail)` em `frontend/src/actions/members.ts`: só para `activated_at IS NULL`; atualiza `auth.users` (admin API) + `profiles.email`; erros do contrato (contracts §updatePendingMemberEmail)
...
- [ ] T011 [US1] `frontend/src/components/members/MemberList.tsx`: badge "Pendente" (shadcn Badge) para `activated_at === null` + ação "Corrigir e-mail" (visível só em pendentes) chamando `updatePendingMemberEmail`
...
- [ ] T013 [US1] Testes Vitest (seguir convenção de localização dos testes existentes no frontend): validação/normalização de e-mail do `addMember` e idempotência do `preregisterSupabaseUser` (mock do admin client)
...
- [ ] T021 [US2] Webhook `frontend/src/app/api/webhooks/clerk/route.ts`: resolver vínculos pendentes — `UPDATE member_email_links SET linked_user_id = <profile> WHERE email = <email> AND linked_user_id IS NULL`
...
- [ ] T025 [US2] `frontend/src/components/members/MemberList.tsx`: exibir e-mails vinculados por membro (FR-015) + ações vincular (abre LinkEmailDialog) e desvincular; remoção de membro também limpa seus links (já coberto em T009/removeMember? conferir e ajustar `removeMember` para deletar `member_email_links` do membro)
...
- [ ] T027 [US2] Testes Vitest: matriz de casos do `linkMemberEmail`, resolução de `getEffectiveMemberId`, montagem do preview de unificação (mocks de client)
