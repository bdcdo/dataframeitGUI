...
```jsonc
{
  ...
  "text_column": "texto"       // coluna mapeada como texto; usada só pelo export
}
```
...
export interface DocumentMetadata {
  ...
  text_column?: string; // nome da coluna de texto; usado só pelo export
}
...
- Colisão: coluna auxiliar com nome igual a coluna de controle, campo do schema ou `document_text` → `original_<nome>`; persiste colisão → sufixo numérico `_2`, `_3`… Mesmo renomeio no CSV e na aba Documentos. `document_text` só entra no conjunto reservado quando há coluna de texto.
