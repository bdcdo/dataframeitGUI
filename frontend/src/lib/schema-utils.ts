...
// Constantes do SHA-256 em module-level: `K` é imutável e reutilizável entre
// chamadas; `SHA256_H_INIT` é o seed dos hash words, copiado por chamada
// (`new Uint32Array(SHA256_H_INIT)`) porque `H` é mutado durante o cálculo.
...
function sha256Hex(input: string): string {
  const K = SHA256_K;
  const H = new Uint32Array(SHA256_H_INIT);
