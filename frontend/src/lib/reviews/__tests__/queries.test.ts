...
// A escolha da review por célula é de `pickValidCellReviews`, testada em
// review-validity.test.ts.
describe("gabaritoReviews", () => {
  it("query falhada (null) vira Gabarito vazio", () => {
    expect(gabaritoReviews(null, new Map())).toEqual([]);
  });
});
