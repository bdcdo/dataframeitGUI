...
-- table has one row per (project, user), so machine restarts and horizontal
-- scaling cannot create independent counters.
...
BEGIN
  ...
  INSERT INTO public.llm_rate_limit_buckets (
    ...
    p_project_id, p_user_id, v_now, 1
  )
  ON CONFLICT (project_id, user_id) DO NOTHING
  RETURNING true INTO v_created;

  -- retry_after_seconds is only meaningful when the dispatch is rejected; every
  -- allowed path returns 0 and the caller ignores it (see llm_rate_limiter.py).
  IF v_created THEN
