-- Fecha o par produtor/fechamento da auto-revisão.
--
-- sync_auto_review_assignment_status (migration 20260715190000) serializa o
-- fechamento contra outro fechamento, mas quem CRIA field_reviews não
-- participava da trava e nunca reabria o assignment: o upsert do produtor usa
-- ignoreDuplicates, que ignora a linha existente em vez de voltá-la para
-- 'pendente'. Duas consequências, ambas deixando o documento fora da fila com
-- veredito por fazer:
--
--   1. Determinística: o pesquisador conclui a auto-revisão, depois edita a
--      codificação; um campo passa a divergir do LLM, o stub novo nasce com
--      self_verdict NULL e o assignment continua 'concluido'.
--   2. Corrida: o produtor insere o stub entre o EXISTS e o UPDATE do sync.
--      Sob READ COMMITTED cada statement lê um snapshot novo, então sem trava
--      compartilhada o fechamento não enxerga o campo recém-criado.
--
-- É o mesmo desenho que a arbitragem já tinha: assign_arbitration_if_eligible
-- pega lock_arbitration_assignment e reabre com
-- `DO UPDATE ... WHERE status = 'concluido'`. Aqui a auto-revisão ganha o par
-- equivalente.
BEGIN;
