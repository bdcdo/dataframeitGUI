...
```bash
cd frontend && npm install      # instala o react-doctor pinado
pre-commit install              # da raiz do repo: registra o hook
```
...
- O hook faz `cd frontend` (o react-doctor resolve config e escopo pelo diretório de execução) e usa o binário pinado de `frontend/node_modules`.
