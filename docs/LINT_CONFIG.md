...
> **O gate está inerte hoje (medido em 2026-07-16, react-doctor 0.7.3).** O scan completo não produz nenhum diagnóstico *error*-level — as issues abertas são todas `warning`. Como o hook roda com `--blocking error`, na prática ele não barra commit algum: só voltará a morder se uma regra de severidade `error` passar a disparar. O débito de errors que motivou o gate (o cluster State & Effects) foi pago pelos PRs do épico #239. Endurecer para `--blocking warning` é a decisão natural para o gate voltar a ter efeito, mas custa pagar ou suprimir os warnings restantes antes — trabalho de PR próprio, não decidido aqui.
