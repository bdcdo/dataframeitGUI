...
3. **Mutação manual como juiz** — a afirmação "este guard está coberto" exige mutar o guard e ver o teste falhar (#454: mecanismo certo, propriedade errada — só a mutação revelou). Escolher a mutação pelo erro plausível, não pela remoção completa: asserção de **ordem** não prova **espera**, e no #494 trocar `await user.reload()` por `void user.reload()` mantinha a sequência registrada correta enquanto o bug seguia de pé — para provar que A espera B, segurar B em aberto (promise não resolvida) e asserir que A ainda não aconteceu. Ferramenta dedicada: avaliação na #517.
