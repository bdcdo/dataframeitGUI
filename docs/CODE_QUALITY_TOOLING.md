...
O primeiro scan completo encontrou **113 erros em 5 arquivos**, com **103 (91%) concentrados em `services/llm_runner.py`** — quase todos `union-attr`/`arg-type` da explosão do alias `JSON` recursivo (`int | float | Sequence[JSON] | Mapping[str, JSON] | None`) toda vez que um `dict` cru vindo do LLM é acessado. É o mesmo arquivo já isento de `C901` no `ruff` (`run_llm`, CCN 35) — tipar direito exige o mesmo refactor maior, fora do escopo de um PR de tooling. Por isso `services/llm_runner.py` é isento por inteiro via `[[tool.mypy.overrides]]` (`ignore_errors = true`), do mesmo jeito que o `per-file-ignores` do `ruff` isenta sua complexidade — sem isso, qualquer push que tocasse o arquivo bloquearia nos 103 erros pré-existentes, já que o hook é file-scoped (não line-scoped). Baseline restante, grandfathered pelo escopo file-a-file do hook: **10 erros em 4 arquivos** (`services/auth.py`: 4, `routes/pydantic_routes.py`: 4, `services/pydantic_compiler.py`: 1, `main.py`: 1) — issue [#380](https://github.com/bdcdo/dataframeitGUI/issues/380) rastreia a correção desse baseline.
