...
O gate usa `fallow audit` (script `npm run fallow:audit`, hook de pre-push). O `audit` é purpose-built para PR quality gates: roda dead-code + complexidade + duplicação escopados aos arquivos alterados e retorna um verdict (pass/warn/fail), com exit 1 em fail. O modo default `--gate new-only` faz o grandfathering por design — só achados *introduzidos* pelo changeset afetam o verdict; os herdados são reportados com atribuição new-vs-inherited mas não barram. **Esse grandfathering vale para as severidades default, não para regra que nós promovemos a `error`**: medido na 3.2.0, com a árvore limpa e `dead_code_introduced: 0` / `dead_code_inherited: 2`, promover uma regra a `error` no `.fallowrc.jsonc` levou o verdict a `fail` e o exit a 1. Isso contradiz a doc do fallow, que afirma que "inherited error-severity findings can be present in the JSON output while the verdict remains `pass`" — a doc está errada, e a consequência prática é que **só se promove regra cuja contagem de achados no projeto seja zero hoje** (confira com `npm run fallow`). Também medido: a flag global `--fail-on-issues`, documentada como "promotes `warn` to `error`", é **no-op** no `audit` — um `misplaced-directive` introduzido de propósito manteve verdict `warn` e exit 0 com e sem a flag. Por isso a promoção é regra a regra no bloco `rules` da config, e não via flag. O `audit` não recebe `--base`: sem ele, o fallow usa a merge-base com `origin/main`, o que evita o falso-positivo de comparar contra um `main` local defasado. O scan completo (`npm run fallow`, dead-code + dupes + health juntos) fica para inspeção manual.
...
A v3 rebaixou de `error` para `warn` oito regras de dead-code (entre elas `unused-server-actions` e `unused-component-props`) e ligou a análise de CSS por default — ou seja, o major **afrouxou** o gate em vez de apertá-lo. A resposta está no bloco `rules` do `.fallowrc.jsonc`: 14 regras `warn` foram promovidas de volta a `error`, todas com contagem zero de achados no projeto no momento da promoção. O critério de seleção foi o que pode de fato disparar neste stack — com destaque para a família de diretivas client/server (`invalid-client-export`, `misplaced-directive`, `mixed-client-server-barrel`), que é a classe de bug que quebrou três deploys em silêncio (#404) e motivou o CI próprio do #452. Ficaram deliberadamente em `warn` as 5 regras `css-*` (a análise de CSS não produz saída neste projeto — verificado com um token drift deliberado no `globals.css`; promovê-las não protegeria nada e criaria o risco de o CSS legado barrar todo push se uma versão futura passar a emiti-las) e as regras inertes de Vue/Svelte/Angular/SvelteKit e de catálogos pnpm. O raciocínio de cada uma está no próprio `.fallowrc.jsonc`.

`frontend/.fallowrc.jsonc` (schema oficial do fallow) supressiona os falsos-positivos: `scripts/comentarios-relatorio/**` (`ignorePatterns` — script chamado manualmente via `tsx`, nunca por um entry point do Next) e cada arquivo de primitiva shadcn/Radix em `src/components/ui/` (`ignoreExports`, enumerados um a um — mesmo raciocínio do #230 no react-doctor, ver `docs/LINT_CONFIG.md`). A enumeração é deliberada em vez de um glob `src/components/ui/**`: o diretório também guarda componentes próprios do projeto (`confirm-action-dialog.tsx`, `CopyLinkButton.tsx`) que devem continuar sob checagem normal de dead-code — um glob amplo os esconderia do gate junto com as primitivas geradas. Um terceiro falso-positivo entrou com a promoção de severidade: `ignoreDependencies: ["shadcn", "tailwindcss"]` mata o `dev-dependencies-in-production` desses dois — o `tailwindcss` só aparece via `@import "tailwindcss"` no `globals.css` (build-time, resolvido pelo `@tailwindcss/postcss`) e o `shadcn` é o CLI gerador, sem import nenhum no `src/`; ambos estão corretos em `devDependencies`. Eram os dois únicos achados de regra `warn` do projeto, e sem a supressão a promoção da regra barraria todo push.
