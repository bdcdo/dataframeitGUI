...
Para atribuir new-vs-inherited, o `--gate new-only` cria um worktree do commit-base em `/tmp` (`fallow-audit-base-cache-<hash-config>-<base>`, detached HEAD) e o mantém como cache reutilizável entre pushes — evita re-checkout + reinstalar `node_modules` contra a mesma base. O GC desse cache tem limiar default de 30 dias (por *último uso*), o que faz uma entrada por commit-base se acumular em `/tmp` e poluir o `git worktree list`. Fixamos `audit.cacheMaxAgeDays: 1` no `.fallowrc.jsonc` para varrer no próximo run os caches não usados há mais de 1 dia, preservando o reuso na base do dia (`0` desligaria o sweep — não usar).
