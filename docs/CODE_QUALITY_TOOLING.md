...
| `pre-push` | todo `git push` (pesado, grafo/tipos) | typecheck · e2e-smoke (Playwright) · lint:types · fallow audit · semgrep · backend-pytest · mypy | new-only / file-scoped (ver cada um); pytest roda a suíte inteira; e2e-smoke pula sem `.env.e2e` |
...
`@playwright/test` (devDependency), specs em `frontend/e2e/*.smoke.spec.ts` (`dashboard.smoke`, `config-guard.smoke`, `lottery.smoke`), config em `frontend/playwright.config.ts`. A autenticação usa Clerk Testing Tokens por ticket (`@clerk/testing`) em vez de UI ou senha — resolve, na íntegra, o problema levantado na issue [#107](https://github.com/bdcdo/dataframeitGUI/issues/107): sem essa peça, qualquer agent/CI que tentasse validar um fluxo logado travava no redirect para `accounts.dev`, o domínio de login do Clerk, sem forma documentada de gerar sessão sem passar pela UI.

Até a issue [#306](https://github.com/bdcdo/dataframeitGUI/issues/306), essa suíte existia mas não rodava sozinha — só quando alguém lembrava de digitar `npm run test:e2e`. Isso deixa um gate real vulnerável a apodrecer sem aviso, exatamente o tipo de dependência de memória que o resto desta stack foi desenhada para eliminar (ver "O problema" acima). O hook `e2e-smoke` fecha essa lacuna: roda no pre-push, mesmo padrão dos outros hooks pesados, escopado a qualquer `.ts`/`.tsx` alterado em `frontend/`. Os specs já fazem `test.skip()` por papel quando as env vars de `.env.e2e` (`E2E_COORDINATOR_EMAIL` etc., não versionado — ver `.env.e2e.example`) não estão definidas — mas isso não é o mesmo que custo zero: o boot do `webServer` (`npm run dev`) e a troca do Testing Token do Clerk (`global.setup.ts`, via `CLERK_SECRET_KEY`/`NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`) são incondicionais e vêm de `.env.local`, que todo checkout funcional já tem — só os `test.skip()` de cada papel dependem de `.env.e2e`. Ou seja: checkouts e worktrees sem `.env.e2e` (inclusive as efêmeras que agentes criam) não são bloqueados pelo hook, mas ainda pagam o custo de subir o servidor e autenticar contra o Clerk dev antes de pular os specs.

O hook seta `PLAYWRIGHT_PRE_PUSH=1`, que `playwright.config.ts` usa para desviar do comportamento "modo dev" do config em três pontos: `baseURL` muda para uma porta dedicada (`3100`, em vez da `3000` padrão), `webServer.reuseExistingServer` vira `false` e `workers` vira `1`. A porta dedicada existe porque `reuseExistingServer: false` sozinho não bastaria: o Playwright não distingue "servidor de outra worktree/branch" de "servidor legítimo" — com `reuseExistingServer:false`, qualquer coisa já escutando na porta configurada faz o `webServer` abortar com erro, em vez de isolar a run. Sem uma porta própria, isso bloquearia o push sempre que o próprio desenvolvedor já tivesse `npm run dev` aberto na porta `3000` para a mesma branch (um fluxo comum — outros comandos deste doc, como `npm run scan`, também assumem um dev server já rodando), não só no caso de "worktree errada" que motivou a mudança. Isolar por porta resolve esse caso comum; resta apenas o risco residual (raro) de duas worktrees-irmãs empurrando ao mesmo tempo e disputando essa mesma porta dedicada. `workers: 1` existe porque o modo dev roda os 3 arquivos de spec em paralelo, o que reintroduziria entre arquivos o mesmo burst de `clerk.signIn()` contra o tenant dev que a #198/PR #294 já corrigiu só dentro de um arquivo. Nenhuma dessas mudanças afeta `npm run test:e2e` rodado manualmente, que continua otimizado para iteração local. Continua valendo, aqui como no `backend-pytest`, a decisão de manter o gate pré-merge nos hooks locais em vez de GitHub Actions — sem branch protection no plano free, um workflow de Actions não bloquearia merge.
...
A cobertura de hoje é deliberadamente rasa (3 specs de fumaça, focados nos fluxos sensíveis a auth/RLS que motivaram o #107 originalmente) — expandi-la é trabalho incremental, fora do escopo da #306, e cabe melhor como issues pontuais por fluxo do que como uma expansão única.
