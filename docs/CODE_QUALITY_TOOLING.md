...
Até a issue [#306](https://github.com/bdcdo/dataframeitGUI/issues/306), essa suíte existia mas não rodava sozinha — só quando alguém lembrava de digitar `npm run test:e2e`. Isso deixa um gate real vulnerável a apodrecer sem aviso, exatamente o tipo de dependência de memória que o resto desta stack foi desenhada para eliminar (ver "O problema" acima). O hook `e2e-smoke` fecha essa lacuna: roda no pre-push, mesmo padrão dos outros hooks pesados, escopado a qualquer `.ts`/`.tsx` alterado em `frontend/`. Os specs já fazem `test.skip()` quando as env vars de `.env.e2e` (`E2E_COORDINATOR_EMAIL` etc., não versionado — ver `.env.e2e.example`) não estão definidas, então checkouts e worktrees sem credenciais de teste local — inclusive as worktrees efêmeras que agentes criam para isolar tarefas — pulam o hook em vez de falhar; o custo de subir o dev server e autenticar os papéis no Clerk dev só é pago onde `.env.e2e` de fato existe. Continua valendo, aqui como no `backend-pytest`, a decisão de manter o gate pré-merge nos hooks locais em vez de GitHub Actions — sem branch protection no plano free, um workflow de Actions não bloquearia merge.
