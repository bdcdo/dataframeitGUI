...
O hook seta `PLAYWRIGHT_PRE_PUSH=1`, que `playwright.config.ts` usa para desviar do comportamento "modo dev" do config em dois pontos: `webServer.reuseExistingServer` vira `false` (o modo dev reaproveita qualquer servidor já escutando na porta, o que em worktrees paralelas — convenção padrão deste repo — pode significar validar o código de outra branch em vez do que está sendo empurrado) e `workers` vira `1` (o modo dev roda os 3 arquivos de spec em paralelo, o que reintroduziria entre arquivos o mesmo burst de `clerk.signIn()` contra o tenant dev que a #198/PR #294 já corrigiu só dentro de um arquivo). Nenhuma das duas mudanças afeta `npm run test:e2e` rodado manualmente, que continua otimizado para iteração local. Continua valendo, aqui como no `backend-pytest`, a decisão de manter o gate pré-merge nos hooks locais em vez de GitHub Actions — sem branch protection no plano free, um workflow de Actions não bloquearia merge.
