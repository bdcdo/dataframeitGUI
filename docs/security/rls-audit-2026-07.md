...
O caller de comparação envia os IDs das respostas visíveis e declara se aquela gravação conclui sua atribuição. `submit_compare_review` bloqueia o documento, valida projeto, identidade efetiva e respostas escolhidas, grava o snapshot filtrado, cria o review e atualiza o assignment na mesma transação. A conclusão é calculada no frontend contra a visão canônica sem filtros temporários; `mark_compare_doc_reviewed` cobre a declaração explícita quando não resta campo divergente, e remover uma equivalência reabre apenas o assignment concluído de quem a removeu. O cliente não grava mais o status em uma tarefa assíncrona posterior ao commit.

Uma comparação concluída é histórico da rodada, e o índice parcial `assignments_one_active_comparacao_per_doc` admite uma única comparação ativa por documento. As duas RPCs que poderiam tirar um assignment de `concluido` — `submit_compare_review`, quando o caller declara que a gravação não conclui a atribuição, e `remove_response_equivalence` — consultam antes `compare_doc_has_other_active_assignment` e preservam o parecer antigo quando o documento já foi re-sorteado. Sem essa guarda, a regressão reinseriria a linha no predicado do índice e a violação de unicidade abortaria a transação inteira, derrubando junto o review e os deletes já gravados.
